...
class UserPledgeModel extends Model{
	//表名
	protected $table = 'user_pledge';
	/**
	 * 质押列表
	 */
	public function pledgeList(){
		...
		//查询符合条件的数据
		$pledgeList = $this->field('users.*,user_total.balance,user_total.total_balance')->join('user_total','users.id = user_total.uid','left')->where($where)->order('users.id','desc')->select()->toArray();
		...
		return array(
			'where'     =>	$param,
			'pledgeList'  =>	$source,//数据
			'count'     => count($pledgeList),
			'page'      =>	$page,//分页
		);
	}
	/**
